...
class screen:
    ...
    def __init__(self,LCD_RD,LCD_WR,LCD_RS,LCD_CS,LCD_RST,LCD_D0,LCD_D1,LCD_D2,LCD_D3,LCD_D4,LCD_D5,LCD_D6,LCD_D7):
        ...
        self.WR_value = 1 << self.LCD_WR
        self.CS_value = 1 << (self.LCD_CS-32)
        self.CD_value = 1 << (self.LCD_RS-32)
        self.RD_value = 1 << self.LCD_RD
        self.RST_value = 1 << self.LCD_RST
        self.WR_off = self.WR_value ^ 0xFFFFFFFF
        self.CS_off = self.CS_value ^ 0xFFFFFFFF
        self.CD_off = self.CD_value ^ 0xFFFFFFFF
        
        
        self.RD_off = self.RD_value ^ 0xFFFFFFFF
        self.RST_off = self.RST_value ^ 0xFFFFFFFF
        ...
        self.GPIO_OUT_REG = 0x3FF44004        #GPIO 0-31 output register
        self.GPIO_OUT_W1TS_REG = 0x3FF44008   #GPIO 0-31 output register_W1TS
        self.GPIO_OUT_W1TC_REG = 0x3FF4400C   #GPIO 0-31 output register_W1TC
        self.GPIO_IN_REG= 0x3FF4403C          #GPIO 0-31 input register

        self.GPIO_OUT1_REG = 0x3FF44010       #GPIO 32-39 output register
        self.GPIO_OUT1_W1TS_REG = 0x3FF44014  #GPIO 32-39 output bit set register
        self.GPIO_OUT1_W1TC_REG = 0x3FF44018  #GPIO 32-39 output bit clear register 
        self.GPIO_IN1_REG= 0x3FF44040         #GPIO 32-39 input register
        self.done_reset=0
        ...
        self.D_MASK=self.mapF8(0xFF)
        self.D_MASK_off = self.D_MASK ^ 0xFFFFFFFF
        self._MC = 0x2A
        self._MP = 0x2B
        self._MW = 0x2C
        self._SC = 0x2A
    ...
    @micropython.viper
    def CD_COMMAND(self):
        GPIO_OUT = ptr32(self.GPIO_OUT1_REG)
    ...
    def map_32(self,d):
        return (
            0
            | ((d & (1 << LCD_D0)) >> (self.LCD_D0 - 0))
            | ((d & (1 << LCD_D1)) >> (self.LCD_D1 - 1))
            | ((d & (1 << LCD_D2)) >> (self.LCD_D2 - 2))
            | ((d & (1 << LCD_D3)) >> (self.LCD_D3 - 3))
            | ((d & (1 << LCD_D4)) >> (self.LCD_D4 - 4))
            | ((d & (1 << LCD_D5)) >> (self.LCD_D5 - 5))
            | ((d & (1 << LCD_D6)) >> (self.LCD_D6 - 6))
            | ((d & (1 << LCD_D7)) >> (self.LCD_D7 - 7))
        )
...
#         sleep_us(300) # PICO could need >300 us of temporisation in the command to respond for the readID() function
    ...
    def setAddrWindow(self,x1:int,y1:int,x2:int,y2:int):
        CS_value = int(ptr32(self.CS_value))
        CS_off = int(ptr32(self.CS_off))
        D_MASK_off= int(ptr32(self.D_MASK_off))
        GPIO_OUT = ptr32(self.GPIO_OUT_REG)
        GPIO_OUT1 = ptr32(self.GPIO_OUT1_REG)
        GPIO_OUT1[0] = GPIO_OUT1[0] & CS_off   #CS_active
        self.WriteCmd(self._SC)
        self.write8(x1>>8)
        self.write8(x1)
        self.write8(x2>>8)
        self.write8(x2)
        GPIO_OUT1[0] = GPIO_OUT1[0] | CS_value   #CS_idle
        GPIO_OUT1[0] = GPIO_OUT1[0] & CS_off   #CS_active 
    ...
    def read16bits(self):
        ...
        ret=self.READ_8()
        #print("READ_8 ret val:",end=str(hex(ret)))
        #print("")
        #all MIPI_DCS_REV1 style params are 8-bit
        sleep_us(1)    #1us should be adequate
        lo=self.READ_8()
        #print("READ_8 lo val:",end=str(hex(lo)))
        #print("")
        return (ret << 8) | lo
    ...
    def reset(self):
        GPIO_OUT = ptr32(self.GPIO_OUT_REG)
        GPIO_OUT1 = ptr32(self.GPIO_OUT1_REG)
        CS_value = int(ptr8(self.CS_value))
        RD_value = int(ptr8(self.RD_value))
        WR_value = int(ptr8(self.WR_value))
        RST_value = int(ptr8(self.RST_value))
        RST_off = RST_value ^ int(0xFFFFFFFF)
        CS_off = CS_value ^ int(0xFFFFFFFF)
        RD_off = RD_value ^ int(0xFFFFFFFF)
        WR_off = WR_value ^ int(0xFFFFFFFF)
        self.CS_IDLE()
        self.RD_IDLE()
        self.WR_IDLE()
        self.RESET_IDLE()
        self.RESET_ACTIVE()
    #     RESET_IDLE()
        GPIO_OUT[0] = GPIO_OUT[0] | RST_value
...
#        **********************Initialization sequence for ILI9341***********************************
    ...
    def balayage(self,h:int,end:int,w:int,color:int):
        WR_value = int(ptr32(self.WR_value))
        WR_off = int(ptr32(self.WR_off))
        D_MASK_off= int(ptr32(self.D_MASK_off))
        ...
        while (h>0):
            ...
            while(end!=0):
                GPIO_OUT[0] = (GPIO_OUT[0] & D_MASK_off) | color_hi
                GPIO_OUT[0] = GPIO_OUT[0] & WR_off
                GPIO_OUT[0] = GPIO_OUT[0] | WR_value
                GPIO_OUT[0] = (GPIO_OUT[0] & D_MASK_off) | color_lo
                GPIO_OUT[0] = GPIO_OUT[0] & WR_off
                GPIO_OUT[0] = GPIO_OUT[0] | WR_value
                end-=1
            h-=1
    ...
    def drawPixel(self,x:int,y:int,color:int):
        ...
        CS_value = int(ptr32(self.CS_value))
        CS_off = int(ptr32(self.CS_off))
        CD_value = int(ptr32(self.CD_value))
        CD_off = int(ptr32(self.CD_off))
        D_MASK_off= int(ptr32(self.D_MASK_off))
        ...
        GPIO_OUT1 = ptr32(self.GPIO_OUT1_REG)
        GPIO_OUT1[0] = GPIO_OUT1[0] & CS_off   #CS_active
        GPIO_OUT1[0] = GPIO_OUT1[0] & CD_off              #CD_COMMAND
        GPIO_OUT[0] = (GPIO_OUT[0] & D_MASK_off) | _SC_h
        GPIO_OUT[0] = GPIO_OUT[0] & WR_off
        GPIO_OUT[0] = GPIO_OUT[0] | WR_value
        GPIO_OUT[0] = (GPIO_OUT[0] & D_MASK_off) | _SC_l
        GPIO_OUT[0] = GPIO_OUT[0] & WR_off
        GPIO_OUT[0] = GPIO_OUT[0] | WR_value

        #CD_DATA
        GPIO_OUT1[0] = GPIO_OUT1[0] | int(ptr32(self.CD_value))            #CD pin on
        #write8F(x_hi)
        GPIO_OUT[0] = (GPIO_OUT[0] & D_MASK_off) | x_hi
        GPIO_OUT[0] = GPIO_OUT[0] & WR_off
        GPIO_OUT[0] = GPIO_OUT[0] | WR_value
        #write8F(x_lo)
        GPIO_OUT[0] = (GPIO_OUT[0] & D_MASK_off) | x_lo
        GPIO_OUT[0] = GPIO_OUT[0] & WR_off
        GPIO_OUT[0] = GPIO_OUT[0] | WR_value
        #write8F(x_hi)
        GPIO_OUT[0] = (GPIO_OUT[0] & D_MASK_off) | x_hi
        ...
        GPIO_OUT[0] = (GPIO_OUT[0] & D_MASK_off) | _SP_h
        GPIO_OUT[0] = GPIO_OUT[0] & WR_off
        GPIO_OUT[0] = GPIO_OUT[0] | WR_value
        GPIO_OUT[0] = (GPIO_OUT[0] & D_MASK_off) | _SP_l
        GPIO_OUT[0] = GPIO_OUT[0] & WR_off
        GPIO_OUT[0] = GPIO_OUT[0] | WR_value
        ...
        GPIO_OUT1[0] = GPIO_OUT1[0] | int(ptr32(self.CD_value))            #CD pin on
        #write8F(y_hi)
        GPIO_OUT[0] = (GPIO_OUT[0] & D_MASK_off) | y_hi
        GPIO_OUT[0] = GPIO_OUT[0] & WR_off
        GPIO_OUT[0] = GPIO_OUT[0] | WR_value
        #write8F(y_lo)
        GPIO_OUT[0] = (GPIO_OUT[0] & D_MASK_off) | y_lo
        GPIO_OUT[0] = GPIO_OUT[0] & WR_off
        GPIO_OUT[0] = GPIO_OUT[0] | WR_value
        #write8F(y_hi)
        GPIO_OUT[0] = (GPIO_OUT[0] & D_MASK_off) | y_hi
        ...
        GPIO_OUT[0] = (GPIO_OUT[0] & D_MASK_off) | _MW_h
        GPIO_OUT[0] = GPIO_OUT[0] & WR_off
        GPIO_OUT[0] = GPIO_OUT[0] | WR_value
        GPIO_OUT[0] = (GPIO_OUT[0] & D_MASK_off) | _MW_l
        GPIO_OUT[0] = GPIO_OUT[0] & WR_off
        GPIO_OUT[0] = GPIO_OUT[0] | WR_value
